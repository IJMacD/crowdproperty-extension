...
const cpInboxSingle =
  /https:\/\/www.crowdproperty.com\/account\/messaging\/(\d+)/;
...
function renderUI() {
  ...
  if (fieldSet) {
    fieldSet.disabled =
      (isDownloadPage ? state.pageMessageCount : state.savedMessageCount) === 0;
  }
...
function getMessages() {
  ...
  for (const el of document.querySelectorAll(".p-4")) {
    const id = +(
      el.querySelector("a")?.href.substring(cpInbox.length + 1) || NaN
    );
...
function getSingleMessage() {
  ...
  if (match) {
    const id = +match[1];
    const subject =
      document.getElementsByClassName("section-title")[1].textContent || "";
    /** @type {HTMLElement} */
...
getCurrentTab().then((tab) => {
  ...
  if (cpInboxSingle.test(tab.url || "")) {
          ...
          .then((results) => {
            for (const { result: message } of results) {
              if (message) {
                ...
                divSingle && (divSingle.style.display = "none");
                divIntro.firstElementChild.textContent =
    ...
    [btnEmlZip, btnMbox, btnMboxZip, btnCsv, btnCsvZip].forEach((btn) => {
      btn &&
...
function downloadMessages(messages, format, zipped) {
  if (format === "eml") {
    ...
    for (const message of messages) {
      ...
      mboxList.push(
        `From ${fromAddr} ${asctime(new Date(message.date))}\n${makeMessage(
          message,
          thread
        )}`
      );
...
function makeMessage(message, thread = []) {
  ...
  let threadHeaders = "";
  if (thread.length > 0) {
    threadHeaders += `References: ${thread
      .map((m) => getMessageID(m))
      .join(" ")}\n`;
    threadHeaders += `In-Reply-To: ${getMessageID(
...
function rfc2822(date) {
  ...
  return `${DAYS[date.getDay()]}, ${date.getDate()} ${
    ...
    date.getMinutes()
  )}:${pad2(date.getSeconds())} ${offsetSign}${pad2(offsetHours)}:${pad2(
    offsetMinutes
  )}`;
}
/**
...
function asctime(date) {
  return `${DAYS[date.getDay()]} ${MONTHS[date.getMonth()]} ${date
    .getDate()
    .toString()
    .padStart(2, "0")} ${pad2(date.getHours())}:${pad2(
    date.getMinutes()
  )}:${pad2(date.getSeconds())} ${date.getFullYear()}`;
}
...
function asctimeUTC(date) {
  return `${DAYS[date.getUTCDay()]} ${MONTHS[date.getUTCMonth()]} ${date
    .getUTCDate()
    .toString()
    .padStart(2, "0")} ${pad2(date.getUTCHours())}:${pad2(
    date.getUTCMinutes()
  )}:${pad2(date.getUTCSeconds())} ${date.getUTCFullYear()}`;
}
